    ...
    try:
        with lock:
            try:
                ...
                with open(f"scripts/{txt}_done.txt", "a", encoding="utf-8") as x:
                    x.write(f"{phone} - {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
            except Exception as e:
                with open(f"scripts/{txt}_error.txt", "a", encoding="utf-8") as error_file:
                    error_file.write(f"{phone} - {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
